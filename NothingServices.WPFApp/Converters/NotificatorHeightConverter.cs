...
public sealed class NotificatorHeightConverter : IMultiValueConverter
{
    ...
    public object? Convert(object?[]? value, Type? targetType, object? parameter, CultureInfo? culture)
    {
        ...
        var valid = double.TryParse(value[0]?.ToString()?.Trim().Replace(",", "."), culture, out var value1);
        valid &= double.TryParse(value[1]?.ToString()?.Trim().Replace(",", "."), culture, out var value2);
