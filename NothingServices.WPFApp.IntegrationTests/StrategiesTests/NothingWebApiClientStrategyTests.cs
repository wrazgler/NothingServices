...
public class NothingWebApiClientStrategyTests
{
    ...
    private static async Task StopApp(int beforeDelay = 10000, int afterDelay = 2000)
    {
        ...
        await Process.Start("docker", "container remove -f -v wpf_nothing_web_api_client_strategy_test_postgres_nothing_web_api_db")
            .WaitForExitAsync();
        ...
        await Process.Start("docker", "image remove -f wpf_nothing_web_api_client_strategy_test_nothing_web_api")
            .WaitForExitAsync();
        await Process.Start("docker", "volume remove -f wpf_nothing_web_api_client_strategy_nothing_services_wpf_app_test_postgres_nothing_web_api_db")
            .WaitForExitAsync();
